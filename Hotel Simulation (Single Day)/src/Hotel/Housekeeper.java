...
/** Maintains information about a hotel housekeeper and their schedule. Their schedule <br>
 * consists of an ordered list of rooms they are assigned and a set of times they will <br>
 * start cleaning each assigned room respectively */
public class Housekeeper {
	...
	public void appendRoom(Room room) {
		...
		int start= Math.max(makespan, room.getCheckOut()) + 1;
		addRoom(room, start);
	}
	...
	public void addRoom(Room room, int startTime) {
		if (room == null) throw new IllegalArgumentException("Room is null");
		if (startTime <= room.getCheckOut()) throw new IllegalArgumentException("Room not checked out");
		int endTime= startTime + room.getCleanTime() - 1;
